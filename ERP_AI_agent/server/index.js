...
app.post("/api/warehouses", async (req, res) => {
  ...
  try {
    ...
    if (err.code === "ER_DUP_ENTRY")
      return res
        .status(409)
        .json({ error: `倉庫編號 ${w.warehouse_id} 已存在` });
    throw err;
  }
});
...
app.post("/api/products", async (req, res) => {
  ...
  try {
    ...
    if (err.code === "ER_DUP_ENTRY")
      return res.status(409).json({ error: `品號 ${p.sku_id} 已存在` });
    throw err;
  }
});
  ...
  try {
    await pool.execute(
      ...
      [
        ...
        v.sku_id,
        v.batch_no || null,
        v.manufactured_date || null,
        v.expiry_date || null,
        v.available_qty || 0,
        v.reserved_qty || 0,
        v.safety_stock || 0,
        v.unit || "箱",
        v.inventory_status || "normal",
      ],
    );
    res.json({ ok: true });
  } catch (err) {
    if (err.code === "ER_DUP_ENTRY")
      return res.status(409).json({ error: "相同倉庫/SKU/批號已存在" });
    throw err;
  }
});
...
app.get("/api/production", async (req, res) => {
  const [rows] = await pool.execute(
    "SELECT * FROM production_batches ORDER BY planned_finish_date",
  );
  res.json(rows);
});
  ...
  try {
    await pool.execute(
      ...
      [
        ...
        b.qty || 0,
        b.uom || "箱",
        b.planned_finish_date || null,
        b.actual_finish_date || null,
        b.expiry_date || null,
        b.batch_status || "planned",
        b.plant || null,
      ],
    );
    res.json({ ok: true });
    ...
    if (err.code === "ER_DUP_ENTRY")
      return res.status(409).json({ error: `批次 ${b.batch_no} 已存在` });
    throw err;
  }
});
...
app.post("/api/customers", async (req, res) => {
  ...
  try {
    ...
    if (err.code === "ER_DUP_ENTRY")
      return res
        .status(409)
        .json({ error: `客戶編號 ${c.customer_id} 已存在` });
    throw err;
  }
});
...
app.get("/api/orders", async (req, res) => {
  ...
  const [items] = await pool.execute(
    "SELECT * FROM order_items ORDER BY order_id, line_no",
  );
  const itemMap = {};
  items.forEach((i) => {
  ...
  try {
    ...
    await conn.execute(
      ...
      [
        o.order_id,
        o.customer_id || null,
        o.customer_name_snapshot || null,
        o.channel_snapshot || null,
        o.region_snapshot || "北區",
        o.delivery_address || null,
        o.required_delivery_date || null,
        o.delivery_window_start || null,
        o.delivery_window_end || null,
        o.priority || null,
        o.risk || null,
    ...
    for (const [idx, item] of items.entries()) {
      const lineTotal =
        (Number(item.ordered_qty) || 0) * (Number(item.unit_price) || 0);
      await conn.execute(
    ...
    await conn.rollback();
    if (err.code === "ER_DUP_ENTRY")
      return res.status(409).json({ error: `訂單編號 ${o.order_id} 已存在` });
    throw err;
  } finally {
    conn.release();
  }
...
app.delete("/api/orders/:id", async (req, res) => {
  await pool.execute("DELETE FROM sales_orders WHERE order_id=?", [
    req.params.id,
  ]);
  res.json({ ok: true });
});
...
app.get("/api/shipments", async (req, res) => {
  ...
  const [items] = await pool.execute(
    "SELECT * FROM shipment_items ORDER BY shipment_id",
  );
  const itemMap = {};
  items.forEach((i) => {
    (itemMap[i.shipment_id] ||= []).push(i);
  });
  res.json(
    shipments.map((s) => ({ ...s, items: itemMap[s.shipment_id] || [] })),
  );
});
  ...
  try {
    ...
    await conn.execute(
      ...
      [
        ...
        s.carrier || null,
        s.vehicle_no || null,
        s.planned_dispatch_at || null,
        s.actual_dispatch_at || null,
        s.eta || null,
    ...
    for (const item of items) {
      ...
      if (item.inventory_id && qty > 0) {
        ...
        const [[inv]] = await conn.execute(
          "SELECT warehouse_id, sku_id, batch_no FROM inventory WHERE id=?",
          [item.inventory_id],
        );
        if (inv) {
          await conn.execute(
    ...
    if (err.code === "ER_DUP_ENTRY")
      return res
        .status(409)
        .json({ error: `出貨單號 ${s.shipment_id} 已存在` });
    throw err;
  } finally {
    conn.release();
  }
...
app.delete("/api/shipments/:id", async (req, res) => {
  await pool.execute("DELETE FROM shipments WHERE shipment_id=?", [
    req.params.id,
  ]);
  res.json({ ok: true });
});
...
app.post("/api/stock-movements", async (req, res) => {
  ...
  try {
    ...
    // 同步更新 inventory
    if (m.inventory_id) {
      const delta = ["in", "release"].includes(m.movement_type)
        ? qty
        : -Math.abs(qty);
...
if (fs.existsSync(dist)) {
  ...
  app.all(/^\/.+$/, (_req, res) => res.sendFile(path.join(dist, "index.html")));
}
