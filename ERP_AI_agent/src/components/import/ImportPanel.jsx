...
export default function ImportPanel({ onImport }) {
  ...
  async function handleFile(e) {
    ...
    if (errors.length) {
      setStatus({ type: 'error', msg: `CSV 解析錯誤：${errors.slice(0, 3).join('；')}` });
      return;
    }
    if (!data.length) {
    ...
    setStatus({ type: 'ok', msg: `已匯入 ${data.length} 筆${def?.label || ''}資料` });
  ...
  return (
    <div className="panel" style={{ padding: 16 }}>
      ...
      {status && (
        <Alert type={status.type === 'ok' ? 'info' : 'error'} onClose={() => setStatus(null)}>
          {status.msg}
        </Alert>
      )}
      <p className="footer-note">
        訂單欄位：order_id, sku_id, item, customer_name, channel, region, quantity, required_delivery_date, priority, risk<br />
        ⚠ 若 CSV 無 region 欄位，系統將以「北區」作為 MVP 假設值，需由 ERP 正式欄位取代。
      </p>
    </div>
  );
}
