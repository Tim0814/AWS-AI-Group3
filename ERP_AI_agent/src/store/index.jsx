...
function computeAllocatable(inventory, production) {
  ...
  inventory.forEach((r) => {
    map[r.sku_id] =
      (map[r.sku_id] || 0) + Number(r.available_qty) - Number(r.reserved_qty);
  });
  production
    .filter((p) => p.batch_status !== "completed")
    .forEach((r) => {
...
export function runEngine(state) {
  ...
  const safetyMap = {};
  inventory.forEach((r) => {
    safetyMap[r.sku_id] = Math.max(
      safetyMap[r.sku_id] || 0,
  ...
  return orders.map((order) => {
    ...
    const itemResults = items.map((item) => {
      ...
      const prodBatch = production.find(
        (p) => p.sku_id === sku && p.batch_status !== "completed",
      );
      if (order.required_delivery_date && prodBatch?.planned_finish_date) {
        checks.deliveryOk =
          prodBatch.planned_finish_date.slice(0, 10) <=
      ...
      if (!checks.skuExists) {
        ...
        exceptionType = "品號不一致";
      } else if (!checks.deliveryOk) {
        suggestion = "exception";
        exceptionType = "交期衝突";
      } else if (!checks.supplyOk) {
        suggestion = "exception";
        exceptionType = "數量不足";
      } else if (!checks.safetyOk) {
        suggestion = "warning";
      }
      ...
      if (!checks.skuExists) {
        ...
        reasons.push(
          `可分配量 ${avail} ${checks.supplyOk ? "≥" : "<"} 需求 ${qty}`,
        );
        if (!checks.safetyOk)
          reasons.push(`出貨後庫存 ${afterStock} 低於安全庫存 ${safety}`);
    ...
    const totalEstCost = itemResults.reduce(
      (s, i) => s + (i._engine.estCost || 0),
      0,
    );
    const totalEstValue = itemResults.reduce(
      (s, i) => s + (i._engine.estValue || 0),
      0,
    );
    const allSupplyOk = itemResults.every((i) => i._engine.checks.supplyOk);
    const exceptionType =
      itemResults.find((i) => i._engine.exceptionType)?.engine?.exceptionType ||
      "";
...
function reducer(state, action) {
  switch (action.type) {
    case "SET_ORDERS":
      return { ...state, orders: action.rows };
    case "SET_CUSTOMERS":
      return { ...state, customers: action.rows };
    case "SET_SHIPMENTS":
      return { ...state, shipments: action.rows };
    case "SET_WAREHOUSES":
      return { ...state, warehouses: action.rows };
    case "SET_PRODUCTS":
      return { ...state, products: action.rows };
    case "SET_INVENTORY":
      return { ...state, inventory: action.rows };
    case "SET_PRODUCTION":
      return { ...state, production: action.rows };
    ...
    case "UPDATE_ORDER":
      return {
        ...
        orders: state.orders.map((o) =>
          o.order_id === action.payload.order_id
            ? { ...o, ...action.payload }
            : o,
        ),
      };
    case "DELETE_ORDER":
      return {
        ...state,
        orders: state.orders.filter((o) => o.order_id !== action.id),
      };
    case "SET_REVIEW":
      return {
        ...state,
        orders: state.orders.map((o) =>
          o.order_id === action.id ? { ...o, review_status: action.status } : o,
        ),
      };
    case "SET_ORDER_REVIEWS":
      return {
        ...state,
        reviews: { ...state.reviews, [action.order_id]: action.rows },
      };
    case "IMPORT_ORDERS": {
      ...
      return {
        ...
        _importInfo: { added: newOrders.length, skipped },
      };
    }
    ...
    case "UPDATE_CUSTOMER":
      return {
        ...
        customers: state.customers.map((c) =>
          c.customer_id === action.payload.customer_id
            ? { ...c, ...action.payload }
            : c,
        ),
      };
    case "DELETE_CUSTOMER":
      return {
        ...state,
        customers: state.customers.filter((c) => c.customer_id !== action.id),
      };
    ...
    case "PATCH_SHIPMENT_STATUS":
      return {
        ...
        shipments: state.shipments.map((s) =>
          s.shipment_id === action.id
            ? { ...s, delivery_status: action.status }
            : s,
        ),
      };
    case "DELETE_SHIPMENT":
      return {
        ...state,
        shipments: state.shipments.filter((s) => s.shipment_id !== action.id),
      };
...
export function StoreProvider({ children }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .catch(() =>
        dispatch({
          type: "SET_ERROR",
          msg: "無法連線至後端 API，請確認 npm run server 已啟動",
        }),
      )
    ...
    try {
      switch (action.type) {
        ...
        case "SET_REVIEW":
          ...
          dispatch(action);
          // 重新載入該訂單審核歷程
          fetchReviews(action.id)
            .then((rows) =>
              dispatch({
                type: "SET_ORDER_REVIEWS",
                order_id: action.id,
                rows,
              }),
            )
        ...
        case "IMPORT_ORDERS": {
          ...
          await Promise.all(
            newRows.map((r) => addOrder({ ...r, review_status: "pending" })),
          );
          dispatch(action);
          break;
        }
  ...
  return (
    <Ctx.Provider value={{ state, dispatch: apiDispatch }}>
      {children}
    </Ctx.Provider>
  );
}
